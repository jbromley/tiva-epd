...
EPD_type *EPD_create(EPD_size size,
		     ...
		     int busy_pin,
		     SPI_type *spi) {
    info("EPD_create(%d, %d, %d, %d, %d, %d, %p)\n", size, panel_on_pin, 
	 border_pin, discharge_pin, reset_pin, busy_pin, spi);
...
void EPD_begin(EPD_type *epd) {
    ...
    info("  wait for COG to become ready\n");
    ...
    if (!dc_ok) {
	info("  !! charge pumps not ready\n");
	epd->status = EPD_DC_FAILED;
	power_off(epd);
	return;
    }
    info("  -> charge pumps ready\n");
...
{
    info("EPD_image(%p, %p, %p)", epd, old_image, new_image);
...
static void frame_fixed(EPD_type *epd, uint8_t fixed_value, EPD_stage stage) {
    info("frame_fixed(%p, %d, %d)\n", epd, fixed_value, stage);
...
static void frame_fixed_repeat(EPD_type *epd, uint8_t fixed_value, 
			       EPD_stage stage) 
{
    info("frame_fixed_repeat(%p, %d, %d)\n", epd, fixed_value, stage);
...
static void one_line(EPD_type *epd, uint16_t line, const uint8_t *data, uint8_t fixed_value, const uint8_t *mask, EPD_stage stage) {
    info("one_line(%p, %d, %p, %d, %p, %d)\n", epd, line, data, fixed_value, mask, stage);
